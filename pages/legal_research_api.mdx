...
To connect to the EventStream, you need to send a `POST` request to the endpoint and listen to the events of the `EventSource`, each event has a type ( *response*, *considerationDetails*, *confidence*, *error*, ** ), the last event is of type *confidence*.

There are 3 parameters to add in the body of the `POST` request: 
...
Here is an example of an implementation in typescript (in this example we use the `sse.js` package to force the POST request at the creation of the EventSource):
