...
// Base type for all consideration details
...
// Special type for OtherInternal with different structure
...
// Union type of all possible consideration detail types
...
The `considerationDetails` field contains an array of source documents that were used to generate the response. Each consideration detail has a specific type and structure.
...
##### Consideration Detail Types

###### Article Types

**ParagraphOrArticle** - Federal law articles and paragraphs
```typescript
export type ParagraphOrArticleDetails = ConsiderationDetailsBase & {
  label: ConsiderationType.ParagraphOrArticle;
  title: string;
...
**CantonalParagraphOrArticle** - Cantonal law articles
```typescript
export type CantonalParagraphOrArticleDetails = ConsiderationDetailsBase & {
  label: ConsiderationType.CantonalParagraphOrArticle;
  title: string;
...
**CommunalParagraphOrArticle** - Communal law articles
```typescript
export type CommunalParagraphOrArticleDetails = ConsiderationDetailsBase & {
  label: ConsiderationType.CommunalParagraphOrArticle;
  title: string;
...
**Commentary** - Legal commentaries (Legalis or Omnilex based on permissions)
```typescript
export type CommentaryDetails = ConsiderationDetailsBase & {
  label: ConsiderationType.Commentary;
  title: string;
...
**CourtDecision** - Court decisions and rulings
```typescript
export type CourtDecisionDetails = ConsiderationDetailsBase & {
  label: ConsiderationType.CourtDecision;
  title: string;
...
The following types also follow the same structure pattern:
...
**OtherInternal** - Internal file sources with a different structure:
...
```md copy
{
  considerationDetails: [ 
    ...
    {
      label: 'cantonal_paragraph',
      title: 'Strassenverkehrsgesetz des Kantons Zürich',
      relevant_ids: [85],
      url: 'https://www.zh.ch/de/politik-staat/gesetze-beschluesse/gesetzessammlung/zhlex-ls/erlass-7411_01.html',
      article_citations: ['§ 12'],
      article_paragraphs_text: ['Im Kanton Zürich gilt...']
    }
  ]
}
